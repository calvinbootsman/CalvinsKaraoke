...
def build_song_payload(song_title: str) -> tuple[dict[str, object] | None, str | None]:
    ...
    pitch_path = song_dir / "pitch.csv"

    pitch_url = None

    if pitch_path.exists():

        pitch_url = f"{media_server_base_url}/{encoded_song_title}/pitch.csv"
...
def add_song_to_queue(song_title: str) -> None:
    ...
    # Auto-play if queue was empty and nothing is currently playing
    if was_empty and not st.session_state.get("current_song") and not st.session_state.get("is_playing"):
        play_song_at_index(0)
